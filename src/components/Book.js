...
export default {
  template: `<div class="book" draggable="true" @dragend="dragEnd" @dragstart="dragStart(bookId, listId)">
                    <img :src="bookImagePath" :title="bookName" @click="openBook({bookPath, bookCurrentPage, listId, bookId})" />
                    <button title="Remove Book" class="remove-book-btn dred" 
                        @click="removeBook({bookId: bookId,
                                            bookAuthor: bookAuthor,
                                            bookImagePath: bookImagePath,
                                            listId:listId})" />
                    <span class="title" :title="bookName" @click="openBook({bookPath, bookCurrentPage, listId, bookId})">{{ bookName }}</span>
